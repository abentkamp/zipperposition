        ...
        match view t with
          ...
          | DB i ->
            begin match DBEnv.find env i with
              | None ->
                if i >= DBEnv.size env
                then bvar ~ty (i - DBEnv.size env) (* unshift *)
                else bvar ~ty i
              | Some t' ->
                assert (equal (ty_exn t') ty);
                (* unshift open variables,
                   since we removed [length env] binders.
                   This must keep closed terms closed. *)
                unshift ~depth:(DBEnv.size env) (DBEnv.size env) t'
            end
          | Const s -> const ~ty s
          | Bind (s, varty, t') ->
